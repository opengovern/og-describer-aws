...
func getAwsVpcRouteTurbotData(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	if routeData.Route.DestinationCidrBlock != nil {
		...
		akas = []string{"arn:" + metadata.Partition + ":ec2:" + metadata.Region + ":" + metadata.AccountID + ":route-table/" + *routeData.RouteTableID + ":" + *routeData.Route.DestinationCidrBlock}
		...
		akas = []string{"arn:" + metadata.Partition + ":ec2:" + metadata.Region + ":" + metadata.AccountID + ":route-table/" + *routeData.RouteTableID + ":" + *routeData.Route.DestinationIpv6CidrBlock}
	} else {
		title = *routeData.RouteTableID
		akas = []string{"arn:" + metadata.Partition + ":ec2:" + metadata.Region + ":" + metadata.AccountID + ":route-table/" + *routeData.RouteTableID}
	}
